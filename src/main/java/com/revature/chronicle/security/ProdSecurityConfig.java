...
public class ProdSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().cors().configurationSource(getCorsConfigurationSource()).and().authorizeRequests()
				.antMatchers("/*").authenticated().and().oauth2ResourceServer().jwt();
	
	}

	/**
	 * creates a CorsConfigurationSource to be used in the configure method to set
	 * up our CORS permissions. Uses the corsConfigurationProperties object to
	 * accomplish this.
	 * 
	 * @return - the CorsConfigurationSource object used to set Spring Security's
	 *         CORS permissions
	 */
	@Bean
